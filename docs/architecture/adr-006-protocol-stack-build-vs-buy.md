...
| **IEC 60870-5-104 (controlling station)** | **USE LIBRARY** (`lib60870-C`, GPLv3/commercial dual-licensed, **no linking exception** — corrected 2026-08-06, see `license-audit.md` §4 item 12) | Superseded the original clean-room BUILD plan: a `bacnet-stack`-style daemon-per-instance-over-IPC was studied first (Sprint P5 kickoff) and doesn't carry the same clean outcome — no GCC-style exception means the daemon component itself stays GPLv3 with source published on distribution, unlike BACnet MS/TP's daemon. Chosen anyway over the ~6–10 eng-week clean-room build; the purchased-spec prerequisite no longer applies to this path. | daemon + IPC layer + Python driver, estimate pending Sprint P5 scoping (mirrors P7's ~30 eng-day shape) |
