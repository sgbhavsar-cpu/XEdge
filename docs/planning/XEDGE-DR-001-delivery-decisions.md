...
| ~~**Q-8**~~ | ~~XEDGE-482 (SNMP agent): xEdge has no IANA-registered Private Enterprise Number for a custom MIB exposing driver/tag/alarm status. Placeholder OID, standard-MIB-II-only descope, or does a real PEN already exist?~~ | **✅ RESOLVED 2026-07-30 — placeholder OID accepted.** The agent ships against a placeholder/experimental OID subtree, explicitly flagged in the Web UI, docs, and the MIB file itself as needing a real PEN before production polling. Swapping in a real PEN later is a one-constant change, not a redesign | — |
| ~~**Q-9**~~ | ~~XEDGE-482 (SNMP agent) depth: system/driver-health summary only, full per-tag value table, or asset-level (ADR-010) summary?~~ | **✅ RESOLVED 2026-07-30 — system + driver summary accepted.** Uptime, driver count/states, per-instance connectivity, alarm counts by severity — no individual tag values via SNMP this delivery. Matches XEDGE-482's 8-point estimate; a full per-tag table would have needed re-estimating | — |
| ~~**Q-10**~~ | ~~XEDGE-484 (TRAP/INFORM receiver): build against a specific field device/vendor MIB, or as generic, operator-configured OID-to-tag mapping?~~ | **✅ RESOLVED 2026-07-30 — generic/configurable accepted.** Modeled as its own driver type (`snmp_trap_receiver`), same event-driven shape as the existing `mqtt_subscriber` driver — an operator maps trap OIDs to tag_ids in config, no vendor assumed. Verify against a real device later if the HIL pass (Q-6) ends up covering one | — |
...
| 1.2 | 2026-07-30 | Q-8/Q-9/Q-10 added and resolved (SNMP agent PEN/MIB scope, agent MIB depth, TRAP/INFORM receiver mapping model — all three ahead of Sprint C8 planning) |
