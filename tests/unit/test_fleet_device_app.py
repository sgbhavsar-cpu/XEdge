...
async def _enrolled_client(
    tmp_path: Path, registry: DeviceRegistry, default_tenant_id: str, device_id: str = "dev1"
) -> tuple[AsyncClient, AsyncClient, CertificateAuthority, str]:
    ...
    (public_client, device_client, ca, device_token) with `device_id`
    already enrolled. `httpx.AsyncClient` over `ASGITransport`, not
    `TestClient` — see test_fleet_manager_app.py's `_client` docstring for
    why (TestClient's separate event loop can't share asyncpg connections
    with the pytest-asyncio loop `fleet_registry` lives on)."""
    ...
    return public_client, device_client, ca, device_token
...
async def test_heartbeat_requires_a_valid_device_token(
    ...
    _public, device_client, _ca, _token = await _enrolled_client(
        tmp_path, fleet_registry, fleet_default_tenant_id
    )
...
async def test_heartbeat_with_valid_token_updates_registry_and_returns_no_pending_config(
    ...
    _public, device_client, _ca, token = await _enrolled_client(
        tmp_path, fleet_registry, fleet_default_tenant_id
    )
...
async def test_config_push_is_delivered_on_next_heartbeat_and_reported_on_the_one_after(
    ...
    public_client, device_client, _ca, token = await _enrolled_client(
        tmp_path, fleet_registry, fleet_default_tenant_id
    )
...
async def test_rotate_certificate_issues_a_new_certificate_for_the_same_identity(
    ...
    _public, device_client, ca, token = await _enrolled_client(
        tmp_path, fleet_registry, fleet_default_tenant_id
    )
